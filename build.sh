...
if [ -z "$LATEST_VERSION" ]; then
  LATEST_VERSION=autobuild-20140724$LATEST_VERSION`curl 'http://distfiles.gentoo.org/releases/amd64/autobuilds/latest-stage3-amd64.txt' | tail -n 1`
...
mkdir rootfs
#git tag $TAG
#sed -ri "s/LATEST_VERSION=autobuild-20140724/LATEST_VERSION=$TAG/" build.sh
#git add build.sh
#git push origin --tags
(cd rootfs && sudo tar xvjpf ../*.tar.bz2)
sudo docker rmi shift/gentoo-stage3-amd64
